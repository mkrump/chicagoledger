...
App = namedtuple('App', 'bills_api, query, bills, twitter_bot')
...
def tweet(ocd_api, query_params, bills, twitter_bot):
    ...
    if len(new_introductions) > 0:
        for new_introduction in new_introductions:
            try:
                ...
                log.error("handler: {} error:  {}".format(e.error_code, e.msg))
            else:
...
def setup(app_config):
    ...
    return App(ocd_bills_api, app_config.query, bills, twitter_bot)
